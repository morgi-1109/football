...
import {catchError, ignoreElements, map, Observable, of, Subject, take} from "rxjs";
...
export class TeamFixturesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fixtures$ = this.footballSportApiWebService.getFixtures$({last: 10, teamId: this.teamId})
      .pipe(
        map((responseItem) => responseItem.response)
      );
